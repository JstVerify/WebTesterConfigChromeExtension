...
window.onload = () => {
  chrome.runtime.sendMessage({action: 'updateInspector'});
}
...
class Inspector {
  constructor() {
    ...
    this.lastElement = '';
    this.lastElementType = '';
    this.lastElementKeys = false;
    var debug = false;
  }
  ...
  recordAction(XPath) {
    ...
    if (element != null) {
      var XPathType = element.tagName;

      if (debug == true){
        console.log('Element Type: '+XPathType);
    ...
    if (this.lastElement){
      var historicElement = document.evaluate(this.lastElement ,document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
      if (historicElement != null) {
        this.lastElementType = historicElement.tagName;
      } else {
        this.lastElementType = false;
      }

    }

    this.lastElement = this.XPath;
    this.lastElementKeys = false;
    var sensitiveData = false;

    if (this.lastElementType == 'INPUT'){
      if (historicElement != null) {
        this.lastElementKeys = historicElement.value;
        if (historicElement.type){
          if (historicElement.type == 'password'){
            sensitiveData = true;
          }
        }
      }
    ...
    if (debug == true){
      console.log('LastElementKeys: '+this.lastElementKeys)
    }

    chrome.runtime.sendMessage({action: 'xpathCaptured', xpath: XPath, xpathType: XPathType, lastElementKeys: this.lastElementKeys, hidden: sensitiveData});
  }
  ...
  deactivate() {
    ...
    this.recordAction('StopAllActions')
...
function getDefaultURL() {
  chrome.runtime.sendMessage({testURL: [window.location.href]});
}
